#source('/Users/claudiagiambartolomei/MEGA/UGI_Software/tools_locuszoom_temp2.R')

#***### What do I need for minerva?
#***###  plink_path = "/Applications/plink-1.07-mac-intel/plink"
system("module load plink2")
...
#   grid.xaxis(at=args[['xat']],gp=gpar(cex=args[['axisSize']],col=args[['frameColor']],alpha=args[['frameAlpha']]));
  ...
  if (!is.null(denote_markers)) {
    ...
    if (! is.null(refidx)) {
      if (!is.null(args[['refsnpName']])) {
        ...
        grid.refsnp(args[['refsnpName']],metal$pos[refidx],transformation(metal$P.value[refidx]),args[['drawMarkerNames']]);
      } else{
        # Use the actual SNP name. 
        grid.refsnp(refSnp,metal$pos[refidx],transformation(metal$P.value[refidx]),args[['drawMarkerNames']]);
      }
    }
  ...
  #### Added this if I have PP4 info:
   #titlev = args[['pp4_value']];
  if (!is.null(args[['pp4_value']]) && nchar(args[['pp4_value']]) > 0) {
        if (as.numeric(args[['pp4_value']]) < 1) {
            grid.pp4(name="PP4 < 1%", xRange[1]+((xRange[2]-xRange[1])/6), max(c(args[['ymax']]-0.5,transformation(metal$P.value)*1.05),na.rm=T));
         } else if (as.numeric(args[['pp4_value']]) > 99) { 
            grid.pp4(name="PP4 > 99%", xRange[1]+((xRange[2]-xRange[1])/6), max(c(args[['ymax']]-0.5,transformation(metal$P.value)*1.05),na.rm=T));
         } else {
        grid.pp4(name=paste("PP4 = ", args[['pp4_value']], "%", sep=""), xRange[1]+((xRange[2]-xRange[1])/6), max(c(args[['ymax']]-0.5,transformation(metal$P.value)*1.05),na.rm=T));
         }
  }
  ####
...
pp4_value = "",
  ...
  unit=1000000,                         # bp per unit displayed in plot # Default was 1000000, changed it to 1000
...
#with plink:
temp.dir <- '/sc/orga/projects/roussp01a/Claudia_TMP/temp/'
   if (!file.exists (temp.dir)) dir.create(temp.dir)
temp.file <- paste(temp.dir, 'region_ld', sep="")
if ('temp.file.code' %in% names(args)) {
  temp.file.code <- as.character(args[['temp.file.code']]) ##VVV
  temp.file <- paste(temp.dir, temp.file.code, sep = '')
}

message('Ref SNP is ', refSnp)
#### my.cmd <- paste(plink_path, " --noweb --silent --bfile /hpc/users/giambc02/scripts/locuszoom/EUR/chr", args[['chr']], " --ld-snp ", refSnp, " --chr ", args[['chr']], " --from-bp ",  args[['startBP']], " --to-bp ", args[['endBP']], " --r2 --ld-window-r2 0 --ld-window 999999 --ld-window-kb 99999 --out ", temp.file,  sep="")

# if have I/D format instead of alleles must change the reference 
#if (any(grep("[ID]", metal$MarkerName))) {
#   refEUR= "/sc/orga/projects/roussp01a/resources/locuszoom/EUR/INDEL_ID/"
#   } else {
#   refEUR= "/sc/orga/projects/roussp01a/resources/locuszoom/EUR/" 
#   }

refEUR= "/sc/orga/projects/roussp01a/resources/locuszoom/EUR/"

# Extract correct name of INDEL in 1000 Genomes: this is a bit desperate and may be wrong!
if (grepl("[ACTG]|[ID]",refSnp)) {

    refSNPpos= gsub(":$", "", gsub("[ACTG]|[ID]", "", refSnp))
    my.cmd.findREF = paste("grep ", refSNPpos, " ",  refEUR, "chr", args[['chr']], ".bim", " | awk '{print $2}'", sep="")
    refSnpLD = system(my.cmd.findREF, intern=TRUE)
    print(paste("SNP found in ref genome: ", refSnpLD, sep=""))
    if (refSnp != refSnpLD) warning("refSNP found in genome does not match alleles of refSNP plotted!")
    
 } else {
    refSnpLD = refSnp
}

my.cmd <- paste("plink --noweb --silent --bfile ", refEUR, "chr", args[['chr']], " --ld-snp ", refSnpLD, " --chr ", args[['chr']], " --from-bp ",  args[['startBP']], " --to-bp ", args[['endBP']], " --r2 --ld-window-r2 0 --ld-window 999999 --ld-window-kb 99999 --out ", temp.file,  sep="")
print(my.cmd)
system(my.cmd)
...
# after reading, clean files
if (file.exists(paste(temp.file, ".ld", sep=""))) file.remove(paste(temp.file, ".ld", sep=""))
if (file.exists(paste(temp.file, ".nosex", sep=""))) file.remove(paste(temp.file, ".nosex", sep=""))
if (file.exists(paste(temp.file, ".log", sep=""))) file.remove(paste(temp.file, ".log", sep=""))
